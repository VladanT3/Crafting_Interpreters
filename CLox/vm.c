	...
	for (;;) {
		...
		switch (instruction = READ_BYTE()) {
			...
			case OP_JUMP_IF_FALSE:
				...
				break;
			case OP_JUMP:
				uint16_t offset1 = READ_SHORT();
				frame->ip += offset1;
				break;
			case OP_LOOP:
				uint16_t offset2 = READ_SHORT();
				frame->ip -= offset2;
				break;
			case OP_CALL:
			...
			case OP_GET_UPVALUE:
				...
				break;
			case OP_SET_UPVALUE:
				uint8_t slot3 = READ_BYTE();
				*frame->closure->upvalues[slot3]->location = peek(0);
				break;
			...
			case OP_INVOKE:
				...
				int arg_count1 = READ_BYTE();
				if (!invoke(method, arg_count1)) {
					return INTERPRET_RUNTIME_ERROR;
				}
				frame = &vm.frames[vm.frame_count - 1];
				break;
